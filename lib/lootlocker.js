...
function updateLeaderboard(loadAllScores=false) {
  getScores(loadAllScores ? 100 : SCORES_ON_LEADERBOARD).then(scores => {
...
window.addEventListener('load', () => {
  login(() => {
    updateLeaderboard(location.href.endsWith('leaderboard.html'));
  });
});

setInterval(() => {updateLeaderboard(location.href.endsWith('leaderboard.html'))}, 60000);